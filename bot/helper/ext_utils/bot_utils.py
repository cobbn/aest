...
def get_readable_message():
    ...
    for download in list(download_dict.values())[STATUS_START:STATUS_LIMIT+STATUS_START]:
        msg += f"<b>{download.status()} »</b> <u>{escape(f'{download.name()}')}</u>\n"
        if download.status() not in [MirrorStatus.STATUS_SPLITTING, MirrorStatus.STATUS_SEEDING]:
            ...
            msg += f"\n<b>ᴜsᴇʀ:</b> {download.message.from_user.mention}"
...
async def checking_access(user_id, button=None):
    ...
    if isExpired:
        ...
        if button is None:
            button = ButtonMaker()
        button.ubutton('ᴄᴏʟʟᴇᴄᴛ ᴛᴏᴋᴇɴ', tinyfy(short_url(f'https://telegram.me/{bot_name}?start={token}')))
        return f'ʏᴏᴜʀ ᴛᴏᴋᴇɴ ʜᴀs ᴇxᴘɪʀᴇᴅ, ᴘʟᴇᴀsᴇ ᴄᴏʟʟᴇᴄᴛ ᴀ ɴᴇᴡ ᴛᴏᴋᴇɴ. \n<b>ɪᴛ ᴡɪʟʟ ᴇxᴘɪʀᴇ ᴀғᴛᴇʀ{time_str}</b>!', button
    return None, button
